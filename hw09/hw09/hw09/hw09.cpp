...
//  hw09
//
//  Created by Danny Tan on 11/25/16.
...
public:
    ...
    void levelOrderTraversal() const{
        ...
        q.push(root);
        while (!q.empty()) { //goes through each line
            Node* temp = q.front();     //stores the parent node onto queue
            q.pop();
            cout << temp->element<< " ";
            if (temp->left) {       //stores left element and prints it out
                q.push(temp->left);
            }
            
            if (temp->right) {  // stores right element and prints it out
                q.push(temp->right);
            }
        }
    }
    ...
    double average_node_depth() const {
        return depthSum(root,0) / numOfNodes (root);
    }
    ...
    void stringy() {
        ...
        inOrder(root, queueOfNodes);
        Node* temp = root;
        while (num != 0) {
            insert(queueOfNodes.front() -> element, temp);
            queueOfNodes.pop();
            num --;
        }
        root = temp;
...
private:
    ...
    void rangedPrint(int low, int high, BinaryNode<Comparable>* node) const {
        if ( node == nullptr )
            return;
        
        if ( node -> element > low) {
            rangedPrint(low, high, node ->left);
        }
        if ( node -> element >= low && high >= node-> element) {
            cout <<  node->element << endl;
        }
        
        if ( high > node -> element ) {
            rangedPrint(low, high, node -> right);
        }
    }
    

    void inOrder(BinaryNode<Comparable>*& node, queue<BinaryNode<Comparable>*>& q){
        if (node == nullptr) {
            return;
        }
        if (node -> left) {
    ...
    double depthSum (BinaryNode<Comparable>* node, int depth) const {
        if (node == nullptr) {
            return 0;
        }
    ...
    int numOfNodes (BinaryNode<Comparable> * node) const {
        if (node == nullptr) {
            return 0;
        }
